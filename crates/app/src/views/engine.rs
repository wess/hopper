...
pub struct Setup {
    ...
    pub busy: bool,
    /// A managed engine the user can start with a click.
    pub can_start: bool,
}
...
pub fn describe(e: &EngineStatus) -> Setup {
    ...
    match e.state {
        ...
        Stopped if e.managed => Setup {
            ...
            title: "Start Hopper's engine".into(),
            body,
            hint,
            busy: false,
            can_start: true,
        ...
        Unsupported => Setup {
            ...
            title: "Hopper can't run its own engine here".into(),
            body,
            hint,
            busy: false,
            can_start: false,
...
mod tests {
    ...
    fn a_starting_engine_reads_as_setup_in_flight() {
        let s = describe(
            &status(EngineState::Starting, "vz", "Downloading Hopper's engine… 40%").managed(true),
        );
        assert!(s.busy);
        assert!(!s.can_start);
    ...
    fn a_stopped_managed_engine_offers_a_start() {
        let s = describe(&status(EngineState::Stopped, "vz", "not running").managed(true));
        assert!(s.can_start);
        assert!(!s.busy);
    }
    ...
    fn an_unsupported_engine_explains_and_offers_no_start() {
        let s = describe(&status(EngineState::Unsupported, "vz", "cannot run here").managed(true));
        assert!(!s.can_start);
        assert!(!s.busy);
